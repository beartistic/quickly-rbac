...
class StrategyController extends BaseController
{
    ...
    public function getList()
    {
        ...
        $params = array();
        list($total, $records, $columns) = Strategy::getQuery($page, $pageSize, $params);
        
        $totalPages = ceil($total / $pageSize);
        return \View::make("product.strategy.list", array(
            'breadcrumb' => Strategy::getBreadcrumb(),
            'columns' => $columns,
            'records' => $records,
            'total' => $total,
            'page'  => $page,
    ...
    public function postQuery($params=array())
    {
        ...
        $totalPages = ceil($total / $pageSize);
        return \View::make("product.strategy.query", array(
            'columns' => $columns,
            'records' => $records,
            'total' => $total,
            'page'  => $page,
    ...
    {
        if (!empty($params)) {
            return Strategy::getQuery($this->defaultPage, 
                $this->defaultPageSize, $params);            
        }
        
        $tableColumns = Input::get('tableColumns', '');
        $params = $this->getInputParams($tableColumns);
        list($total, $records, $columns) = Strategy::getQuery($this->defaultPage, $this->defaultPageSize, $params);
        return \View::make("product.strategy.inputgroup", array(
            'columns' => $columns,
            'fillData' => $records,
        ));
    }
    ...
    public function postUpdate()
    {
        ...
        Strategy::postUpdate(array('id',$id), $params);
        $this->setEmpty($params);
        return $this->postQuery($params);
    }
    ...
    public function postDelete()
    {
        $tableColumns = Input::get('tableColumns', '');
        $params = $this->getInputParams($tableColumns);
        Strategy::postDelete(array('id'=>$params['id']));
