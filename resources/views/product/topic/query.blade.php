...
$categoryCollect = Topic::getFiledDistinct("id,category_name");
$categoryMap = Topic::format2Array($categoryCollect,'id','category_name');
?>
          <table class="table table-bordered b-t b-light">
            <thead>
              <tr>
                ...
                <th><?php if(isset(Topic::$fieldsMap[$v])) echo Topic::$fieldsMap[$v]['name']; else echo $v; ?></th>
                <?php endforeach; ?>
            ...
            <?php foreach ($records as $k=> $v): ?>
              <tr>
                ...
                <?php if(in_array($k1, array('create_time','update_time','operator'))) : ?>
                <?php continue; ?>
                <?php elseif ($k1 == "sub_title"): ?>
                <td title='<?= $v1 ?>'><?= $v1 ?></td>
                <?php elseif($k1 == "image"): ?>
                <td>
                <div class="previewimg">
                    <img src="<?= $v->image ?>" style="width:30px;height:30px;margin:1px;"/>
                </div>
                </td>
                <?php elseif($k1 == "address"): ?>
                <td>
                <?php if (!empty($v->address)){ ?>
                <a target="_blank" href="<?= $v->address ?>">查看</a>
                <?php } ?>
                </td>
                <?php else : ?>
                <td><?= $v1 ?></td>
          ...
          <div style="display:none;" class='row'>
             	...
             	<input type="hidden" id="requestUrl"  value="<?= url("topic/postquery") ?>">
