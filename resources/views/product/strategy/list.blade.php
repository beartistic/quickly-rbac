...
var hanlder = function(that){
  var requestUrl = "<?= url("strategy/postquery") ?>";
...
////Add
var hanlder = function(that){
  var requestUrl = "<?= url("strategy/postadd") ?>";
  var params = getParams();
  var content = {"name":"img_item_map","value":$(".wysiwyg_content").val()};
  params.push(content);
...
var hanlder = function(that){
  ...
  var requestUrl = "<?= url("strategy/postrow") ?>";
  var tableColumns = $("#tableColumns").val();
...
var hanlder = function(that){
  var requestUrl = "<?= url("strategy/postupdate") ?>";
  var tableColumns = $("#tableColumns").val();
  var params = getParams();
  var content = {"name":"img_item_map","value":$(".wysiwyg_content").val()};
  params.push(content);
...
////Remove Confirm
var hanlder = function(that) {
  var requestUrl = "<?= url("strategy/postdelete") ?>";
  var tableColumns = $("#tableColumns").val();
  var params = getParams();
  $.ajax({
    "url": requestUrl,
...
var hanlder = function(that) {
	...
	$("#query_form").find("input[name='id']").val(id);
	//set modal
	$("#mymodel .model_body_txt").text("您确定要删除当前记录吗？");
}
