...
var hanlder = function(that){
  var requestUrl = "<?= url("user/postquery") ?>";
...
////Add
var hanlder = function(that){
  var requestUrl = "<?= url("user/postadd") ?>";
  var params = getParams();
...
var hanlder = function(that){
  ...
  var requestUrl = "<?= url("user/postrow") ?>";
  var tableColumns = $("#tableColumns").val();
...
var hanlder = function(that){
  var requestUrl = "<?= url("user/postupdate") ?>";
  var tableColumns = $("#tableColumns").val();
  var params = getParams();
  $.ajax({
    "url": requestUrl,
...
////Remove Confirm
var hanlder = function(that) {
  var requestUrl = "<?= url("user/postdelete") ?>";
  var tableColumns = $("#tableColumns").val();
  var params = getParams();
  $.ajax({
    "url": requestUrl,
...
////Remove
var hanlder = function(that) {
	var id = that.parent().parent().find("td:eq(2)").text();
	$("#query_form").find("input[name='username']").val(id);
	//set modal
	$("#mymodel .model_body_txt").text("您确定要删除当前路由吗？");
}
